###################################
#
# pcr_dm_connect.tcl
#
###################################
#
# Request DServ to connect to the PCR and request the PCR
#   to send DM data.
#
# Written  29jun06  Started with pcr_wfsc_connect.tcl
#

proc pcr_dm_connect { parent_win } {

    global AO_GUI_HOME
    source $AO_GUI_HOME/../pcr/tcl/pcr/pcr_globals.tcl

    global reply_data

#    if { $TCS_Connected } {
#	set status \
#	    [tk_messageBox -parent $parent_win -icon question -type yesno \
#		 -message "Tcs stage is currently connected\n Are you sure want to reconnect?" ]
#	if { $status == "yes" } {
#	    tcs_close $parent_win
#	} else {
#	    return 0
#	}
#    }
    set status [catch { PCR_Cmd DM_Connect } msg ]

    return 0
}
