...
#
	...
	set error_msg "Wrong number of args in call to tcs_set. \
...
#
	...
	set error_msg "Too many args in call to tcs_set. Found [expr [llength $args]+1]"
	tk_messageBox -parent . -title "Argument Error" -type ok -icon error \
	    -message $error_msg
	return -1
    }
#
	...
	if { $request == "setaaao" || $request == "setrdao" } {
	    ...
	    if { $request == "setaaao" } {
		set value_1 [expr $TCS(ao_off_alt) + $delta_1 ]
		set value_2 [expr $TCS(ao_off_az) + $delta_2 ]
	    }
	    if { $request == "setrdao" } {
		set value_1 [expr $TCS(ao_off_ra) + $delta_1 ]
		set value_2 [expr $TCS(ao_off_dec) + $delta_2 ]
	    }
	    if { $value_1 > 50.0 || $value_1 < -50.0 } {
		set error_msg "Error in tcs_set: Alt/RA too large => $value_1"
		...
		set error_msg "Error in tcs_set: Az/Dec too large => $value_2"
		tk_messageBox -message $error_msg \
		    -parent $parent_win \
		    -icon error -type ok
		return -1
	    ...
	    set string "$request\n$value_1\n$value_2"
	    set status [catch { TCS_Cmd Set $string } msg ]
	    if { $status } {
		set error_msg "Error retrieving requested value from TCS\n $reply_data"
		tk_messageBox -message $error_msg \
		    -parent $parent_win \
		    -icon error -type ok
		return -1
	    ...
	    set value_1 [expr $TCS(alt) + $delta_1 ]
	    set value_2 [expr $TCS(az) + $delta_2 ]
	    set string "$request\n$value_1\n$value_2\n$TCS(rot)"
	    set status [catch { TCS_Cmd Set $string } msg ]
	    if { $status } {
		set error_msg "Error retrieving requested value from TCS\n $reply_data"
	    ...
	    set error_msg "tcs_set: request must be either setaaao or setrdao"
	    tk_messageBox -message $error_msg \
		-parent $parent_win \
		-icon error -type ok
	    return -1
