...
common tss_lun_block, tss_lun, use_tss_server
...
; Force a new connection for each update.
...
; note that this loop does NOT try to re-establish the connection, just to use
; the existing connection.  If we restart the tss server, the only thing to do
; is to kill this script and start over.
...
if (event.id eq event.handler) then begin
    ...
    case uvalue of
      ...
      ; release connection and stand back.
       'snooze' :  begin
	      if tss_lun ne 0 then begin	
              free_lun,tss_lun
              tss_lun=0
...
if ( use_tss_server ne 1 ) then begin
