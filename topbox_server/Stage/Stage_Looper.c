/*
  ...
  This routine starts a process that request the status of the requested
    stage (Indigo or F15) from the Stage motor controller at the given interval.

  Modified  24aug07  tjt  Started with MVP_Looper.c
...
/*================================================================================*
 * Start status looper for the given motor, controlled by an Stage motor controller
 *================================================================================*/
void *Stage_Looper( void *Passed_Info)
{
  /*
  ...
  */
  if ( !Socket->connected ) {
    ...
    */
    ...
    if ( status ) {
      printf("  Stage_Looper: Error connecting to the %s, %s:%d in Socket_OpenClient\n",
  ...
  */
    ...
    */
	...
	Request the current position of the given device
      */
      ...
      if ( status ) {
	if ( *debug ) {
	  printf("  Stage_Looper: Error retrieving position from %s\n", Socket->name);
	...
	continue;
      ...
      /*
	Request status string from the Stage motor driver and update the Data structure
      */
      status = Stage_Status( Socket, Data, *debug);
      if ( status ) {
	if ( *debug ) {
	  printf("  Stage_Looper: Error retrieving status from %s\n", Socket->name);
	  printf("              Connection Closed\n");
	  fflush(stdout);
	}
	(void) Socket_Close( Socket, local_debug);
	...
	continue;
      ...
      if ( local_debug ) {
	printf("  Stage_Looper: Not connected to %s\n", Socket->name );
	fflush(stdout);
      }
	...
	Calculate time spent retrieving the status
      */
      ...
      printf("  Stage_Looper: %s running at %4.1f Hz\n", Socket->name, (float) samples/total_time);
      fflush(stdout);
  ...
  if ( *debug ) {
    printf("  Stage_Master: Stage Server Shutdown\n");
    fflush(stdout);
  }
