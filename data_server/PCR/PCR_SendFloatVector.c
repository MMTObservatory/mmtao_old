...
int PCR_SendFloatVector( DServ_Info *Info, int sockfd, char *name, int *debug )
{
  ...
  */
  ...
  if ( Socket_StringRead( sockfd, request) ) {
    if ( *debug ) {
      printf("  PCR_SendFloatVector: Error reading vector length in Socket_StringRead\n");
      fflush(stdout);
    }
  }
  ...
  */
  ...
  if ( DEBUG ) {
    ...
    if ( !strncmp ( name, "slope_offset_set", strlen("slope_offset_set") ) ||
	 !strncmp ( name, "mod_offset_set", strlen("mod_offset_set") ) ) {
    ...
    } else if ( !strncmp ( name, "mod_rate_set", strlen("mod_rate_set") ) ) {

      printf("    Modulation Rate Vector\n");
      printf("    ----------------------\n");
 ...
 /*
    All commands require a connection to the PCR server, so every time
      PCR_SendFloatVector is called, we will check to make sure we are connected, or
  ...
  */
  if ( !Info->socket->connected ) {
    ...
    */
    ...
    if ( status ) {
      printf("  PCR_SendFloatVector: Error in Socket_OpenClient\n");
      fflush(stdout);
      free(float_vector);
      free(flipped);
      return(-1);
  ...
  */
  if ( !strncmp ( name, "slope_offset_set", strlen("slope_offset_set") ) ) {
    ...
    if ( Socket_StringWrite( Info->socket->sockfd, request, strlen(request)) != strlen(request) ) {
      if ( *debug ) {
	printf("  PCR_SendFloatVector: Error writing slope_offset_set in Socket_StringWrite\n");
	fflush(stdout);
      }
      free(float_vector);
      free(flipped);
  ...
  } else if ( !strncmp ( name, "mod_offset_set", strlen("mod_offset_set") ) ) {
    strcpy( request, "mod_offset_set\n");
    if ( Socket_StringWrite( Info->socket->sockfd, request, strlen(request)) != strlen(request) ) {
      if ( *debug ) {
	printf("  PCR_SendFloatVector: Error writing mod_offset_set in Socket_StringWrite\n");
	fflush(stdout);
      }
      free(float_vector);
      free(flipped);
    ...
    if ( Socket_StringWrite( Info->socket->sockfd, request, strlen(request)) != strlen(request) ) {
      if ( *debug ) {
	printf("  PCR_SendFloatVector: Error writing mod_rate_set in Socket_StringWrite\n");
	fflush(stdout);
      }
      free(float_vector);
      free(flipped);
    ...
    printf("  PCR_SendFloatVector: Unknown request %s\n", name);
    fflush(stdout);
    free(float_vector);
    free(flipped);
    return(-1);
  ...
  */
  ...
  if ( !status ) {
    ...
    */
    printf("  PCR_SendFloatVector: Reply not received\n");
    fflush(stdout);
    ret_val = -1;
    ...
    */
    if ( Socket_StringRead( Info->socket->sockfd, reply) ) {
      printf("  PCR_SendFloatVector: Error reading reply in Socket_StringRead\n");
      fflush(stdout);
      free(float_vector);
      free(flipped);
      pthread_mutex_unlock( &PCR_CMD_Lock);
    ...
    */
    ...
    if ( !strncmp (reply, "ok", strlen("ok") ) ) {
      ...
      if ( *debug ) {
        printf("  PCR_SendFloatVector: Reply = %s\n", Reply);
	fflush(stdout);
      }
      ret_val = 0;
	...
	fflush(stdout);
      ...
      printf("  PCR_SendFloatVector: Unknown reply from PCR: %s\n", Reply);
      fflush(stdout);
      ret_val = -1;
